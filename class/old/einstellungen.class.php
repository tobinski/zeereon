...
class etempus_einstellungen extends etempus {
		...
		***********************************************/		
		public function __construct(){
				...
				if ($this->uid=="1"){
						$this->submenu = array(	"{config_pw_lang}" 	=> 	"",
												"{config_ansatz}" 	=> 	"ansatz",
												"{config_user}" 	=> 	"user",
												/*"{config_misc}"		=>	"misc"*/
												);
				} else {
		...
		**************************************/
		...
		public function start($message=false,$fail=false){
				...
				//seite ausgeben
				$tpl_arr = array( "dd_style"	=> $this->build_style_dd(),
								  "dd_lang"=>$langlist,
								  "msg"=>$msg );
				$tpl=new template;
		...
		*******************************************/
		private function ansatz($message=false,$fail=false){
				...
				foreach ($this->table_array("ansatz") as $row){
						...
						$aktiv_link = "<a href='?modul=ansatz_toggle&id={$row['id']}'>$aktiv</a>";
						
						$item=new template;
						$item->assign(	array(	"db_aktiv"	=> $aktiv_link,
												"db_name"	=> $row['name'],
												"id"		=> $row['id'],
												"db_wert"	=> $row['wert'], ));
				...
				$tpl->assign( array (	"ansatz_liste"	=>$list,
										"msg"			=> $msg,));
				$this->html=$tpl->parse("config_ansatz.tpl");
		}
		...
		**************************************/
		private function user_start($message=false,$fail=false){
				...
				$tpl->assign( array (	"db_user_list"	=>$list,
										"msg"			=> $msg,));
				$this->html=$tpl->parse("config_user_start.tpl");
		}
		...
		****************************************/
		...
		private function user_new($fail=false){
				...
				if ($this->getvar("step")!="2"){
						...
						$this->html=$tpl->parse("config_user_new.tpl");
				}
		...
		***************************************/
		...
		public function user_del(){
				...
				if ($this->getvar("bestaetigt")!="true"){
					...
					$tpl->assign_id();
					$this->html=$tpl->parse("config_user_del.tpl");
				}
				//sonst löschen
				else {
						//prüfen ob id in der session die selbe wie in der form ist
		...
		************************************/
		public function user_detail($message=false,$fail=false){
				...
				$tpl->assign_id();
				$this->html=$tpl->parse("config_user_detail.tpl");
